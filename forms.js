...
document.addEventListener( 'DOMContentLoaded', function() {
    ...
    function formBuild(structure, container) {
        ...
        for (var k in structure) {
            ...
            if ( structure[k].validate.contained ) {
                ...
                getJSON(
                    'lists/' + structure[k].validate.contained + '.json',
                    function (data, optionsId) {
                        container.listOfOptions[optionsId] = data;
                    },
                    structure[k].validate.contained
                );
        ...
        function formSubmit() {
            ...
            postJSON( 'post.php', results, submitResults );
            
            function submitResults(data, args) {
                notLoadingEffect();
                if ( data === 'success' ) {
        ...
        document.querySelectorAll( 'form.the-form[id]' ).forEach( function( form ) {
            form.querySelectorAll( 'form.the-form *[data-autocomplete]' ).forEach( function( input ) {
                ...
                if ( form.listOfOptions && form.listOfOptions[autocompleteId] )
                    ...
                    getJSON( 'lists/' + autocompleteId + '.json', autoCompleteSuggest, input );
    ...
    function getJSON(url, func, args) {
       fetch( url, {
            method: 'GET',
            mode: 'same-origin',
            cache: 'no-cache',
    ...
    function postJSON(url, body, func, args) {
        fetch( url, {
            method: 'POST',
            mode: 'same-origin',
            cache: 'no-cache',
    ...
    document.querySelectorAll( 'form.the-form' ).forEach( function( form ) {
        ...
        getJSON( 'forms/' + form.id + '-fields.json', formBuild, form );
    });
